...
export interface AuthContextValue {
  ...
  loading: boolean;
  signUp: (email: string, password: string, username: string, fullName: string) => Promise<{ error: import('@supabase/supabase-js').AuthError | null }>;
  signIn: (email: string, password: string) => Promise<{ error: import('@supabase/supabase-js').AuthError | null }>;
  signInWithGoogle: () => Promise<{ error: Error | import('@supabase/supabase-js').AuthError | null }>;
  signInWithFacebook: () => Promise<{ error: Error | import('@supabase/supabase-js').AuthError | null }>;
  signOut: () => Promise<void>;
  resetPasswordForEmail: (email: string) => Promise<{ data: object; error: import('@supabase/supabase-js').AuthError | null }>;
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((event, sess) => {
      ...
      if (sess) {
        ...
        if (event === 'SIGNED_IN' || event === 'INITIAL_SESSION') {
          configurePurchases(sess.user.id);
        }
