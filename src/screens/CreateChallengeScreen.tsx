...
import {
  ...
  Alert, Platform, KeyboardAvoidingView, StatusBar,
} from 'react-native';
...
import Step1Basics, { CHALLENGE_TEMPLATES } from './create-challenge/Step1Basics';
import Step2Scoring from './create-challenge/Step2Scoring';
import Step3Rules from './create-challenge/Step3Rules';
import Step4Preview from './create-challenge/Step4Preview';
import { C } from '../theme';
...
export default function CreateChallengeScreen() {
  ...
  function applyTemplate(t: typeof CHALLENGE_TEMPLATES[0]) {
    setName(t.name);
    applyPreset(t.days);
    setScoringModes(t.scoring as ScoringMode[]);
  ...
  async function handleCreate() {
    ...
    if (scoringModes.includes('workouts') && (isNaN(pwVal) || pwVal <= 0)) {
      Alert.alert('Invalid points', 'Points per workout must be a positive number.'); return;
    }
    if (scoringModes.includes('steps') && (isNaN(psVal) || psVal <= 0)) {
      Alert.alert('Invalid points', 'Points per 1,000 steps must be a positive number.'); return;
    }
    if (scoringModes.includes('distance_km') && (isNaN(pkVal) || pkVal <= 0)) {
      Alert.alert('Invalid points', 'Points per km must be a positive number.'); return;
    }
    if (scoringModes.includes('duration_min') && (isNaN(pdVal) || pdVal <= 0)) {
      Alert.alert('Invalid points', 'Points per 30 minutes must be a positive number.'); return;
    }
  ...
  const stepTitles: Record<Step, string> = { 1: 'Basics', 2: 'Scoring', 3: 'Rules', 4: 'Review' };
        ...
        <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
          ...
          {step === 2 && (
            <Step2Scoring
              ...
              isPro={isPro}
              pointsPerWorkout={pointsPerWorkout} setPointsPerWorkout={setPointsPerWorkout}
              pointsPer1000Steps={pointsPer1000Steps} setPointsPer1000Steps={setPointsPer1000Steps}
              pointsPerKm={pointsPerKm} setPointsPerKm={setPointsPerKm}
              pointsPer30min={pointsPer30min} setPointsPer30min={setPointsPer30min}
            />
          )}
          ...
          {step === 3 && (
            <Step3Rules
              isPro={isPro}
              backlogDays={backlogDays} setBacklogDays={setBacklogDays}
              requirePhoto={requirePhoto} setRequirePhoto={setRequirePhoto}
              teamsMode={teamsMode} setTeamsMode={setTeamsMode}
              tieBreak={tieBreak} setTieBreak={setTieBreak}
              isPublic={isPublic} setIsPublic={setIsPublic}
              maxParticipants={maxParticipants} setMaxParticipants={setMaxParticipants}
              renewalType={renewalType} setRenewalType={setRenewalType}
              upgradeVisible={upgradeVisible} setUpgradeVisible={setUpgradeVisible}
              offering={offering} purchase={purchase} restore={restore}
            />
          )}
          ...
          {step === 4 && (
            <Step4Preview
              ...
              scoringModes={scoringModes} tieBreak={tieBreak}
              backlogDays={backlogDays} requirePhoto={requirePhoto}
...
const s = StyleSheet.create({
  ...
  progressRow: {
    ...
    gap: 12, paddingVertical: 16, position: 'relative',
  },
  progressLine: {
  ...
  progressDot: {
    ...
    borderWidth: 1, borderColor: C.border, alignItems: 'center', justifyContent: 'center', zIndex: 1,
  },
  ...
  stepLabel: { textAlign: 'center', fontSize: 13, color: C.muted, fontWeight: '600', marginBottom: 4 },
