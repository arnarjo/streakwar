...
(function () {
  ...
  // league members (gold) — weekly points, ordered desc
  const league = [
  ...
  // 13-week activity heatmap with intensity 0..4
  function buildHeatmap() {
    ...
    for (let w = 0; w < 13; w++) {
      ...
      for (let d = 0; d < 7; d++) {
        ...
        if (r > 0.82) v = 3;
        if (r > 0.93) v = 4;
        if (w >= 9) v = Math.min(4, v + (r > 0.4 ? 1 : 0)); // recent weeks denser (streak)
        days.push(v);
      }
      cols.push(days);
    }
  ...
  const reactionsList = ['🔥','💪','👏','😂']; // shown as icons in UI, mapped below
